...
from app.models import AuditLog, Booking, Client, Compensation, Hotel, Package, Sale, Shooting, User, UserRole
...
class FakeTelegramSession(BaseSession):
    ...
    async def make_request(self, bot, method, timeout=None):
        ...
        if isinstance(method, SendMessage):
            return Message(message_id=9000 + len(self.calls), date=datetime.now(timezone.utc), chat=Chat(id=int(method.chat_id), type='private'), text=method.text)
...
def state_for(tg_id):
    return FSMContext(dp.storage, StorageKey(bot_id=bot.id, chat_id=tg_id, user_id=tg_id))


async def message(tg_id, text):
    global update_id
    update_id += 1
    event = Message(message_id=update_id, date=datetime.now(timezone.utc), chat=Chat(id=tg_id, type='private'), from_user=TelegramUser(id=tg_id, first_name='Test', is_bot=False), text=text)
...
async def callback(tg_id, data):
    global update_id
    update_id += 1
    event = CallbackQuery(id=str(update_id), from_user=TelegramUser(id=tg_id, first_name='Test', is_bot=False), chat_instance='test', data=data, message=Message(message_id=1, date=datetime.now(timezone.utc), chat=Chat(id=tg_id, type='private')))
...
def isolated_database():
    async def setup():
        ...
        async with Session() as session:
            ...
            for tg_id, role in [(ADMIN, 'ADMIN'), (MANAGER, 'MANAGER'), (PHOTO_A, 'PHOTOGRAPHER'), (PHOTO_B, 'PHOTOGRAPHER')]:
            ...
            booking = Booking(hotel_id=hotel.id, client_id=client.id, room='101', shoot_date=date(2026, 9, 15), shoot_time=time(12), package_id=package.id, manager_id=manager.id, photographer_id=photographer.id)
            session.add(booking)
            await session.flush()
...
def test_start_is_idempotent_and_does_not_grant_staff_to_strangers():
    async def scenario():
        ...
        async with Session() as session:
            ...
            assert (await session.execute(select(func.count(UserRole.id)).where(UserRole.user_id == owner.id, UserRole.role == 'OWNER'))).scalar_one() == 1
            assert await roles_of(session, await get_user(session, STRANGER)) == set()
    run(scenario())


@pytest.mark.parametrize('command', ['/add_employee', '/add_hotel', '/add_package', '/set MANAGER_PERCENT 90'])
...
def test_admin_cannot_grant_admin_but_owner_can_repeat_grant():
    ...
    async def scenario():
        await grant(ADMIN)
        async with Session() as session:
            assert 'ADMIN' not in await roles_of(session, await get_user(session, PHOTO_A))
        await grant(OWNER)
        await grant(OWNER)
        async with Session() as session:
            user = await get_user(session, PHOTO_A)
            assert await roles_of(session, user) == {'ADMIN', 'PHOTOGRAPHER'}
            assert (await session.execute(select(func.count(UserRole.id)).where(UserRole.user_id == user.id))).scalar_one() == 2
    run(scenario())
...
def test_manager_and_photographer_screens_and_admin_sales_route():
    async def scenario():
        for tg_id, text in [(MANAGER, '📋 Мои записи'), (MANAGER, '📸 Съёмки'), (MANAGER, '📊 Статистика'), (PHOTO_B, '📸 Мои съёмки'), (PHOTO_B, '📊 Моя статистика'), (PHOTO_B, '👤 Профиль'), (OWNER, '💰 Продажи')]:
...
def test_invalid_sale_quantity_is_rejected(count):
    async def scenario():
        ...
        async with Session() as session:
            assert (await session.execute(select(func.count(Sale.id)))).scalar_one() == 0
    run(scenario())
...
def test_sale_credits_other_photographer_and_uses_database_percent(creator):
    async def scenario():
        ...
        async with Session() as session:
            ...
            assert (await session.execute(select(func.count(AuditLog.id)).where(AuditLog.action == 'sale_created'))).scalar_one() == 1
...
def test_employee_percent_takes_precedence_and_forged_role_is_rejected():
    async def scenario():
        async with Session() as session:
            credited = await get_user(session, PHOTO_B)
            session.add(Compensation(user_id=credited.id, role='PHOTOGRAPHER', sales_percent=23))
            await session.commit()
...
def test_shooting_order_ownership_and_repeated_clicks():
    async def scenario():
        ...
        async with Session() as session:
            ...
            assert (await session.execute(select(func.count(AuditLog.id)))).scalar_one() == 4
    run(scenario())
...
def test_package_prices_reject_invalid_numbers(value):
    async def scenario():
        ...
        async with Session() as session:
            assert (await session.execute(select(func.count(Package.id)))).scalar_one() == 1
