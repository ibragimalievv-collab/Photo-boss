        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                for name,title in [('insights','Контроль бизнеса'),('team','Команда и найм'),('onboarding','Начало работы с Photo Boss'),('workday','Итоги смены'),('development','Развитие фотографа и продажи')]:
                    ...
                    if name == 'team':
                        await page.locator('#hrCreate [name=name]').fill('Кандидат для проверки HR')
                        await page.locator('#hrCreate [name=contact]').fill('@hr_acceptance')
                        await page.locator('#hrCreate [name=source]').fill('Рекомендация')
                        await page.locator('#hrCreate [name=region]').select_option(label='Сочи')
                        await page.locator('#hrCreate button').click()
                        await page.locator('#candidateCard').wait_for()
                        await page.locator('#hrUpdate [name=stage]').select_option('CONTACTED')
                        await page.locator('#hrUpdate [name=reminderAt]').fill('2026-09-24T10:00')
                        await page.locator('#hrUpdate [name=note]').fill('Ручная запись: кандидат согласен на интервью; отправки нет.')
                        await page.locator('#hrUpdate button').click()
                        await page.get_by_text('Ручная запись: кандидат согласен на интервью; отправки нет.',exact=False).last.wait_for()
                        await page.reload()
                        await page.get_by_role('button',name='Кандидат для проверки HR',exact=False).click()
                        assert await page.locator('#hrUpdate [name=reminderAt]').input_value()=='2026-09-24T10:00'
