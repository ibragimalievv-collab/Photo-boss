"""Authenticated Mini App API. No demo records or parallel financial ledger.

Booking/payment/photo mutations stay in the existing bot workflows for this test
release. Schedule, theme and lesson changes are persisted in the existing DB.
"""
...
from .miniapp_security import (
    AccessError, STAFF_ROLES, THEMES, financial_period, parse_shift,
...
ACTIONS = {
    ...
    "employee_fired": "Отключил сотрудника",
    "employee_restored": "Восстановил сотрудника",
...
class MiniApp:
    def __init__(self, engine, bot, lessons, *, tz_name="Europe/Moscow", static_dir=None):
        ...
        self.lessons = [{"slug": lesson.slug, "title": lesson.title, "body": lesson.body,
                         "duration": "Короткий урок"} for lesson in lessons]
    ...
    async def middleware(self, request, handler):
        ...
        if request.path.startswith("/app/"):
            # Inline styles are needed for progress bars, never inline JavaScript.
    ...
    async def me(self, request):
        ...
        return web.json_response({"user": {"id": actor["id"], "telegramId": actor["tg_id"],
            ...
            "timezone": str(self.tz), "mode": "live", "release": "miniapp-v3-test"})
    ...
    async def finance_data(self, conn, actor, start, end, period):
        ...
        for sale in sales:
            ...
            item = employees.setdefault(uid, {"id": uid, "name": sale["employee"], "role": sale["commission_role"], "sales": 0, "commission": 0, "adjustments": 0, "earned": 0})
            item["sales"] += cents(sale["amount"])
            item["commission"] += cents(sale["commission"])
        for row in payroll:
            uid = row["user_id"]
            item = employees.setdefault(uid, {"id": uid, "name": row["name"], "role": "", "sales": 0, "commission": 0, "adjustments": 0, "earned": 0})
            item["adjustments"] += cents(row["amount"])
        ...
        return {"period": period, "from": str(start), "to": str(end), "scope": "self" if own else "company",
                ...
                "note": "Тестовый выпуск. Показаны записи существующего учёта, перед выплатой сверяйте ведомость. Касса — подтверждённые чеки по дате подтверждения. Начислено — комиссии и записи премий/удержаний, не выплаченная зарплата и не чистая прибыль. Продажи показаны до учёта оплаты."}
    ...
    async def shifts_data(self, conn, actor, start, end, *, team=False):
        ...
        items = []
        for row in rows:
            start_at, end_at = as_utc(row["start_at"]).astimezone(self.tz), as_utc(row["end_at"]).astimezone(self.tz)
            attendance = "PLANNED"
            checks = await self.rows(conn, """SELECT started_at FROM shift_check_ins
                ...
                uid=row["user_id"], day=start_at.date())
            outs = await self.rows(conn, """SELECT ended_at FROM shift_check_outs
                ...
                uid=row["user_id"], day=start_at.date())
            if checks:
                attendance = "ENDED" if outs else ("STARTED" if start_at.date() == self.today() else "MISSING_CHECKOUT")
            items.append({"id": row["id"], "userId": row["user_id"], "name": row["name"], "role": row["role"],
                "hotelId": row["hotel_id"], "hotel": row["hotel"], "date": str(start_at.date()), "start": start_at.strftime("%H:%M"),
                "end": end_at.strftime("%H:%M"), "status": row["status"], "attendance": attendance})
        return items
    ...
    async def academy(self, request):
        ...
        known = {lesson["slug"] for lesson in self.lessons}
    ...
    async def complete_lesson(self, request):
        ...
        lesson = next((item for item in self.lessons if item["slug"] == slug), None)
        if lesson is None:
    ...
    async def static_file(self, request):
        ...
        allowed = {"index.html", "config.js", "css/styles.css", "js/app.js", "js/icons.js", "js/domain.js",
                   "js/api.js", "js/telegram.js", "js/academy.js", "assets/icon.svg"}
        # Reuse educational images already shipped in this repository. No private
        # customer files, credentials, demo modules or arbitrary paths are served.
        images = {
        ...
        if name in images:
            path = Path(__file__).parent / "assets" / "training" / images[name]
