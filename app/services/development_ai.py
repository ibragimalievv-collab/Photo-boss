...
async def structured(instructions, content, schema, name, *, limit=5000):
    ...
    try:
        ...
        if body.get('status')!='completed': return {'status':'unavailable'}
        ...
        if not isinstance(result,dict): return {'status':'unavailable'}
...
SUMMARY_SCHEMA=obj({'strengths':STRINGS,'issues':STRINGS,'nextShoot':STRINGS,'diversity':STR,'limitations':STR})
ROLEPLAY_SCHEMA=obj({'reply':STR,'score':{'type':['integer','null']},'errors':STRINGS,'recommendations':STRINGS})
...
async def analyze_batch(images):
    ...
    if response['status']=='completed':
        frames=response['data'].get('frames',[])
        ...
        if len(frames)!=len(expected) or {f.get('photo_id') for f in frames}!=expected or any(not isinstance(f.get(k),str) or len(f[k])>1200 for f in frames for k in FRAME['properties'] if k!='photo_id'):
            return {'status':'unavailable'}
        if any(not isinstance(group,list) or len(group)<2 or any(type(pid) is not int or pid not in expected for pid in group) for group in response['data'].get('duplicates',[])): return {'status':'unavailable'}
    return response


async def summarize_shoot(batches):
    return await structured('Составь план развития фотографа по результатам визуального анализа всех кадров. Сравнение похожих описаний между пакетами — только предположение, а не доказанный дубль. Дай 3–5 конкретных действий на следующую съёмку; не давай оценку сотруднику по защищённым признакам. Не меняй оплату, штрафы, статус съёмки.',
...
async def roleplay(client_type,transcript,finish):
    ...
    return await structured('Ты тренажёр продаж Photo Boss. Правила: '+SALES_RULES+' Играй клиента типа '+scenario['title']+'. '+('Диалог завершён: оцени ошибки сотрудника, score 0–100 и конкретные рекомендации. Не оценивай личные качества.' if finish else 'Ответь как реалистичный клиент, продолжая разговор. score=null, errors и recommendations пустые.'),
