...
def menu(roles):
    items = ['📸 Мои съёмки', '🔄 Моя смена', '📊 Моя статистика', '💰 Мои выплаты', '👤 Профиль'] if 'PHOTOGRAPHER' in roles else []
    if 'MANAGER' in roles:
        items += ['➕ Новая запись', '📋 Мои записи', '📸 Съёмки', '💰 Продажи', '📊 Статистика', '🏆 Премия']
    if {'ADMIN', 'OWNER'} & roles:
        items += ['👥 Сотрудники', '🏨 Отели', '📋 Все записи', '📸 Все съёмки', '💰 Продажи', '💵 Зарплаты/выплаты', '📊 Отчёты', '📦 Пакеты', '⚙️ Настройки', '📜 Аудит']
    if set(ROLES) & roles:
        items += ['🧾 Продажа']
...
async def get_user(session, tg):
    return (await session.execute(select(User).where(User.tg_id == tg))).scalar_one_or_none()


async def roles_of(session, user):
    if user is None or not user.active:
        return set()
    return set((await session.execute(select(UserRole.role).where(UserRole.user_id == user.id))).scalars())
...
async def audit(session, user, action, entity=None, eid=None, details=None):
    ...
    session.add(AuditLog(user_id=user.id if user else None, action=action, entity=entity, entity_id=eid, details=details))
...
async def bootstrap(session, tg, name, username, owner=False):
    ...
    if owner and user.active:
        if (await session.execute(select(Package.id).limit(1))).scalar_one_or_none() is None:
            price = float(await setting(session, 'PHOTO_PRICE', config.photo_price))
            session.add(Package(name='Базовая', price_per_photo=price))
        if (await session.execute(select(Hotel.id).limit(1))).scalar_one_or_none() is None:
