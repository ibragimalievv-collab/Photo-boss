...
let data=null,queue=[],selected=null,transitionVersion='';
const labels={local:'Сохранено локально',syncing:'Синхронизируется',synced:'Синхронизировано',error:'Ошибка'};
const kinds={booking:'Бронь',booking_receipt:'Чек брони',sale_start:'Черновик продажи',sale_receipt:'Чек продажи',sale_counts:'Количество кадров',sale_selected:'Выбранный кадр',sale_complete:'Оформление продажи',shoot_photo:'Кадр полной съёмки',shoot_complete:'Завершение загрузки',checklist:'Контрольный пункт',shoot_transition:'Этап съёмки',shift_report:'Замечание',attendance:'Отметка смены'};
...
const steps={ASSIGNED:['start','Начать съёмку'],PICKED_UP:['start','Начать съёмку'],SHOOTING:['finish','Завершить съёмку'],SHOT:['processing','На обработке'],PROCESSING:['ready','Перенести в продажу']};
function shootingView(){return data.bookings.filter(b=>b.canUpload&&steps[b.shootingStatus]).map(b=>{const [action,label]=steps[b.shootingStatus];return `<form class="panel form-grid section" data-shoot-transition data-sid="${b.shootingId}" data-step="${action}"><h3>Съёмка №${b.id} · ${esc(b.date)} ${esc(b.time)} · комната ${esc(b.room)}</h3>${action==='ready'?field('Почему переносите в продажу?', '<textarea class="input" name="reason" required minlength="3" maxlength="1000" placeholder="Обработка завершена, материалы готовы к показу гостю"></textarea>'):''}<button class="btn primary">${label}</button></form>`;}).join('')||'<p>Нет назначенных съёмок на этих этапах.</p>';}
export function workflowView(){
 const ready=data.bookings.filter(b=>b.status==='READY_FOR_SALE'),b=data.bookings.find(b=>b.id===selected),d=b?.draft;
 const upload=data.bookings.filter(b=>b.status==='READY_FOR_SALE'&&b.saleCompleted&&!b.fullUploaded&&b.canUpload);
 return `<h1>Рабочие операции</h1><div id="workflowNotice" class="notice">${esc(connectionNote())}</div>
 <section class="section"><h2>Мои съёмки</h2><div id="workflowStages">${shootingView()}</div></section>
 ...
 <section class="panel section"><h2>Продажа</h2><p>Сначала загрузите выбранные кадры и завершите продажу. Затем загрузите всю съёмку в течение 48 часов. Начисление фотографу — после продажи и полной загрузки; оплата подтверждается владельцем.</p><select class="input" id="workflowSalePick"><option value="">Выберите готовую съёмку</option>${ready.map(x=>`<option value="${x.id}" ${x.id===selected?'selected':''}>№${x.id} · ${esc(x.date)} · комната ${esc(x.room)}</option>`).join('')}</select>${b?(d&&!d.mine?'<p>Продажу оформляет другой сотрудник.</p>':`<form id="workflowSale" class="form-grid section"><p>Цена кадра по текущему пакету: ${esc(b.price)} ₽. Итог рассчитает сервер.</p>${!d||d.status==='AWAITING_RECEIPT'?field('Чек продажи',file('receipt')):''}${field('Всего кадров в съёмке',input('total',`type="number" min="1" max="10000" required value="${d?.total||''}" ${d?.status==='AWAITING_SELECTED'?'readonly':''}`))}${field('Продано кадров',input('sold',`type="number" min="1" max="10000" required value="${d?.sold||''}" ${d?.status==='AWAITING_SELECTED'?'readonly':''}`))}<p>Уже сохранено выбранных кадров: ${d?.selected||0}. Прикрепите оставшиеся.</p>${field('Выбранные фотографии',file('selected',true,false))}<button class="btn primary">Сохранить продажу и фотографии</button></form>`):''}</section>
 <details class="panel section"><summary>Загрузить полную съёмку</summary>${upload.map(b=>`<p>№${b.id} · ${b.uploadOverdue?'Просрочено · ':''}Загрузить до ${esc(new Date(b.uploadDueAt).toLocaleString('ru-RU'))}</p>`).join('')}${upload.length?`<form id="workflowShoot" class="form-grid">${field('Съёмка',`<select class="input" name="booking">${options(upload)}</select>`)}${field('Фотографии',file('photos',true,false))}<label><input type="checkbox" name="finish"> Все кадры добавлены — завершить загрузку</label><p>Можно добавлять частями. Ставка фотографа фиксируется сервером после завершения полной загрузки.</p><button class="btn primary">Сохранить фотографии</button></form>`:'<p>Нет доступных съёмок для полной загрузки.</p>'}</details>
...
export async function workflowSubmit(form){
 ...
 if(form.matches('[data-shoot-transition]')){
  add('shoot_transition',{shooting:{id:Number(form.dataset.sid)},action:form.dataset.step,reason:f.get('reason')||''});
 }else if(form.id==='workflowBooking'){
  ...
  if(!d||d.status!=='AWAITING_SELECTED')add('sale_counts',{draft,total,sold,price:b.price});
 ...
 if(items.some(item=>pending.some(r=>r.kind===item.kind&&r.status!=='synced'&&JSON.stringify(r.data)===JSON.stringify(item.data)&&!item.blob)))throw new Error('Это действие уже в очереди. Дождитесь синхронизации или повторите операцию в очереди.');
 await enqueueBatch(items);form.reset();queue=await outboxRows();document.querySelector('#workflowQueue').innerHTML=queueView();
}
export async function workflowClick(button){if(button.dataset.workflow==='retry')await retryOperation(button.dataset.key);else {await flushOutbox();await loadWorkflow();document.querySelector('#app').innerHTML=workflowView();}}
...
window.addEventListener('pb-outbox',async()=>{refreshConnection();const el=document.querySelector('#workflowQueue');if(el){queue=await outboxRows();el.innerHTML=queueView();const version=queue.filter(r=>r.kind==='shoot_transition'&&r.status==='synced').map(r=>r.key).join(',');if(version!==transitionVersion){transitionVersion=version;try{await loadWorkflow();const stages=document.querySelector('#workflowStages');if(stages){if(queue.filter(r=>r.kind==='shoot_transition'&&r.status==='synced').at(-1)?.data.action==='ready')document.querySelector('#app').innerHTML=workflowView();else stages.innerHTML=shootingView();}}catch{/* Keep the persisted queue visible until the network returns. */}}}});
