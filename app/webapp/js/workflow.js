...
export function workflowView(){
 ...
 return `<h1>Рабочие операции</h1><div class="notice">${data._offline?`Нет сети. Справочники сохранены ${new Date(data._cachedAt).toLocaleString('ru-RU')}. `:''}Брони, продажи, чеки и фото сохраняются на этом устройстве. Суммы и права проверяются сервером при синхронизации. До статуса «Синхронизировано» операция ещё не учтена.</div>
...
window.addEventListener('pb-outbox',async()=>{const el=document.querySelector('#workflowQueue');if(el){queue=await outboxRows();el.innerHTML=queueView();}});
