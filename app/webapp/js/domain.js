...
export function permissions(roles){const owner=roles.includes('OWNER'),admin=roles.includes('ADMIN');return {financeScope:owner?'all':admin?'today':'self',audit:owner,manageSchedule:owner||admin,manageBookings:owner||admin||roles.includes('MANAGER')};}
...
export function periodRange(period,today=todayKey()){const d=new Date(today+'T12:00:00Z');if(period==='week')return [addDays(today,-((d.getUTCDay()+6)%7)),today];if(period==='month')return [today.slice(0,8)+'01',today];return [today,today];}
...
export function initials(name){return String(name??'').split(/\s+/).filter(Boolean).slice(0,2).map(x=>x[0]).join('');}
// Used only by isolated tests/preview. Live payroll always comes from the API ledger.
export function computeCommission(amountKopeks,frames){if(!Number.isSafeInteger(amountKopeks)||amountKopeks<0||!Number.isInteger(frames)||frames<0)throw new Error('Некорректные данные');const rate=frames>=150?15:10;const bonus=amountKopeks>=2100000?100000+Math.floor((amountKopeks-2100000)/500000)*50000:0;return {rate,commission:Math.round(amountKopeks*rate/100),bonus};}
