...
from ..models import User, UserRole, Hotel, Package, Booking, Shooting, Sale, PayrollEntry, Setting, AuditLog
...
async def etg(m, state):
 try:
  tg_id = int(m.text)
  if not 0 < tg_id < 2**52: raise ValueError
...
async def ename(m, state):
 ...
 await m.answer('Роли через запятую: ADMIN, MANAGER, PHOTOGRAPHER. Назначать ADMIN может только владелец.')
...
async def eroles(m, state, current_roles):
 ...
 async with Session() as session:
  ...
  if user is None:
   ...
   session.add(user)
   await session.flush()
  existing = set((await session.execute(select(UserRole.role).where(UserRole.user_id == user.id))).scalars())
  if existing & {'ADMIN', 'OWNER'} and 'OWNER' not in current_roles:
   return await m.answer('Изменять владельца или администратора может только владелец.')
  user.name = data['name']
  for role in requested - existing:
   session.add(UserRole(user_id=user.id, role=role))
  await audit(session, actor, 'employee_roles_added', 'user', user.id, ','.join(sorted(requested - existing)))
  await session.commit()
 await state.clear()
 await m.answer('Сотрудник сохранён. Существующие роли сохранены; для обновления меню сотруднику нужен /start.')
...
async def hname(m, state):
 ...
 async with Session() as session:
  ...
  session.add(hotel)
  await session.flush()
  await audit(session, await get_user(session, m.from_user.id), 'hotel_created', 'hotel', hotel.id)
  await session.commit()
 await state.clear()
...
async def pprice(m, state):
 try:
  ...
  return await m.answer('Нужна положительная цена до 1 000 000 000, например 400 или 400.50.')
 data = await state.get_data()
 async with Session() as session:
  package = Package(name=data['name'], price_per_photo=price)
  session.add(package)
  await session.flush()
  await audit(session, await get_user(session, m.from_user.id), 'package_created', 'package', package.id)
  await session.commit()
 await state.clear()
...
async def setv(m):
 ...
 if key not in {'PHOTO_PRICE', 'MANAGER_PERCENT', 'PHOTOGRAPHER_PERCENT'}:
  return await m.answer('Ключ: PHOTO_PRICE, MANAGER_PERCENT или PHOTOGRAPHER_PERCENT.')
 try:
  value = number(raw, key, minimum=0.01 if key == 'PHOTO_PRICE' else 0, maximum=1000000000 if key == 'PHOTO_PRICE' else 100)
 except ValueError:
  return await m.answer('Цена должна быть положительной, процент — от 0 до 100. Используйте точку для дробной части.')
 async with Session() as session:
  ...
  await audit(session, await get_user(session, m.from_user.id), 'setting_changed', details=f'{key}={value}')
  await session.commit()
 await m.answer('Настройка сохранена.' + (' Цена действует при создании начального базового пакета; цены существующих пакетов не меняются.' if key == 'PHOTO_PRICE' else ' Новые продажи без персонального процента используют это значение.'))
